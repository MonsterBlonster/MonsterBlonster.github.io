...
var qualityChange;
var quality;
var currentTeapot = teapot(quality);
var num_points = currentTeapot.TriangleVertices.length;
...
onload = function init()  {
    ...
    console.log(currentTeapot.TriangleVertices.length);
...
function teapotRedo(quality, gl, vPosition, vNormal) {
    currentTeapot = teapot(quality);
    currentTeapot.scale(0.5, 0.5, 0.5);
    num_points = currentTeapot.TriangleVertices.length;
    gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(currentTeapot.TriangleVertices), gl.STATIC_DRAW);

    gl.vertexAttribPointer(vPosition, 4, gl.FLOAT, false, 0, 0);
    gl.enableVertexAttribArray(vPosition);
  
    gl.bindBuffer(gl.ARRAY_BUFFER, nBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, flatten(currentTeapot.Normals), gl.STATIC_DRAW);
...
var render = function(){
    ...
    if (qualityChange !== quality) {
        quality = qualityChange;
        teapotRedo(quality, gl, vPosition, vNormal);
      } 
